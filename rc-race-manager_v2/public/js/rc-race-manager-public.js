(function($) {
    ...
    $(document).ready(function() {
        ...
        console.log('RC Race Manager: User logged in:', rcRaceManager.user_logged_in);
        ...
        // Event listener per aprire i dettagli della gara
        $(document).on('click', '.gara-row, .hover-card', function() {
            const garaId = $(this).data('gara-id');
            console.log('Click su gara:', garaId);
            if (garaId) {
                visualizzaDettagliGara(garaId);
            }
        });
    });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.handleLogout = async function() {
        console.log('RC Race Manager: Inizializzazione processo di logout');
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error('Errore di rete durante il logout');
            }

            const data = await response.json();
            console.log('RC Race Manager: Risposta logout:', data);

            if (data.success) {
                alert(data.data.message);
                window.location.href = data.data.redirect || '/';
            } else {
                throw new Error(data.data?.message || 'Errore durante il logout');
            }
        } catch (error) {
            console.error('RC Race Manager: Errore durante il logout:', error);
            alert('Errore durante il logout: ' + error.message);
        }
    };

    // Funzione per visualizzare i dettagli della gara
    window.visualizzaDettagliGara = async function(garaId) {
        console.log('RC Race Manager: Richiesta visualizzazione dettagli per gara:', garaId);
        ...
        try {
            ...
            const data = await response.json();
            console.log('RC Race Manager: Risposta server per dettagli gara:', data);

            if (data.success) {
                // Aggiorna il contenuto del modale
                document.getElementById('dettagliGaraContent').innerHTML = data.data.dettagli_html;
                document.getElementById('listaIscrittiContent').innerHTML = data.data.iscritti_html;

                // Aggiorna pulsanti azioni
                const azioniContainer = document.getElementById('azioniGara');
                ...
                // Pulsante Iscrizione/Cancellazione se l'utente è loggato
                if (rcRaceManager.user_logged_in) {
                ...
                // Mostra il modale
                const modalElement = document.getElementById('modalDettagliGara');
                const modal = new bootstrap.Modal(modalElement);
                modal.show();
            } else {
                console.error('Errore dal server:', data.data?.message);
                alert(data.data?.message || 'Errore nel caricamento dei dettagli della gara.');
            }
        } catch (error) {
            console.error('Errore durante il recupero dei dettagli:', error);
            alert('Errore di rete durante il caricamento dei dettagli della gara.');
        }
    };

    // Funzione per iscriversi a una gara
    window.iscrivitiGara = async function(garaId) {
        if (!rcRaceManager.user_logged_in) {
        ...
        if (confirm('Confermi l\'iscrizione alla gara?')) {
            try {
                ...
                if (data.success) {
                    ...
                    visualizzaDettagliGara(garaId); // Ricarica i dettagli
                } else {
                    throw new Error(data.data?.message || rcRaceManager.strings.iscrizione_error);
                }
            } catch (error) {
                console.error('Errore durante l\'iscrizione:', error);
                alert(error.message || rcRaceManager.strings.network_error);
            }
        }
    };

    // Funzione per cancellare l'iscrizione
    window.cancellaIscrizione = async function(garaId) {
        if (confirm('Sei sicuro di voler cancellare l\'iscrizione?')) {
            try {
                ...
                if (data.success) {
                    ...
                    throw new Error(data.data?.message || rcRaceManager.strings.cancellazione_error);
                }
            } catch (error) {
    ...
    window.esportaPDF = async function(garaId) {
        try {
            const formData = new FormData();
