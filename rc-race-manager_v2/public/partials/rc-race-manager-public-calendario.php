...
$table_iscrizioni_gara = $wpdb->prefix . 'rc_iscrizioni_gara';
...
$gare = $wpdb->get_results("
    SELECT g.*, p.nome as pista_nome, u.display_name as gestore_nome,
           (SELECT COUNT(*) FROM $table_iscrizioni_gara i WHERE i.gara_id = g.id) as num_iscritti
    FROM $table_gare g 
...
error_log('RC Race Manager: Query gare completata - Trovate ' . count($gare) . ' gare');
...
// Se l'utente è loggato, verifica se è un pilota approvato
$pilota_id = 0;
if ($is_logged_in) {
...
error_log('RC Race Manager: Stato utente - Logged in: ' . ($is_logged_in ? 'Si' : 'No') . ', User ID: ' . $current_user_id . ', Pilota ID: ' . $pilota_id);
?>
...
<div class="container">
    ...
    <?php else: ?>
        <?php foreach ($gare as $gara): 
            error_log('RC Race Manager: Visualizzazione gara - ID: ' . $gara->id . ', Nome: ' . $gara->nome);
        ?>
                        ...
                        <div class="col-md-4 text-end">
                            ...
                            <?php if ($is_logged_in && $pilota_id): 
                                $iscritto = $wpdb->get_var($wpdb->prepare(
                                    "SELECT COUNT(*) FROM $table_iscrizioni_gara WHERE gara_id = %d AND pilota_id = %d",
                                    $gara->id, $pilota_id
                                ));
                                error_log('RC Race Manager: Stato iscrizione - Gara ID: ' . $gara->id . ', Pilota ID: ' . $pilota_id . ', Iscritto: ' . ($iscritto ? 'Si' : 'No'));
                            ?>
                                <?php if (!$iscritto): ?>
                                    <button type="button" 
                                            class="btn btn-primary mt-2" 
                                            onclick="iscrivitiGara(<?php echo $gara->id; ?>)">
                                        <i class="fas fa-plus me-2"></i>Iscriviti
                                    </button>
                                <?php else: ?>
                                    <button type="button" 
                                            class="btn btn-danger mt-2" 
                                            onclick="cancellaIscrizione(<?php echo $gara->id; ?>)">
...
<script src="rc-race-manager-calendar.js"></script>